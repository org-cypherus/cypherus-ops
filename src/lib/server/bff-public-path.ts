...
const PUBLIC_CATALOG_PREFIXES = ["v1/plans", "v1/features", "v1/permissions"];

const PUBLIC_CATALOG_METHODS = new Set(["GET", "HEAD"]);
...
function isPublicCatalogRead(path: string, method: string) {
  if (!PUBLIC_CATALOG_METHODS.has(method.toUpperCase())) return false;

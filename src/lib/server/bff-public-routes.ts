...
/** Catalog reads stay anonymous; writes require a CRM session. */
export const PUBLIC_CATALOG_PREFIXES = ["v1/plans", "v1/features", "v1/permissions"] as const;

const CATALOG_PUBLIC_METHODS = new Set(["GET", "HEAD", "OPTIONS"]);
...
function isPublicCatalogRead(path: string, method: string) {
  if (!CATALOG_PUBLIC_METHODS.has(method.toUpperCase())) return false;
...
export function isPublicCrmPath(path: string, method: string) {
  if (path === "v1/companies" && method.toUpperCase() === "POST") return true;
